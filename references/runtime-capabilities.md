...
- 最终放置槽的精确目标宽高、框内容区宽高、外框留边和不规则边缘类型
- 页边组件的背景模式：`transparent-raster` 或 `svg-vector`
- 本章主题色板及来源；纸底固定白色，点睛色可随风格和主题变化
...
- 透明背景不支持：优先生成 SVG/CSS 线稿或使用确定性抠图能力；K/M/L 页边组件不得退化成带白色矩形底的插图卡。仍无法满足时进入 `visual-pending`，不要伪造透明回执。
