...
飞书正文必须使用可阅读的原生块：章节标题、原文段落、会议事项、图片和图注分别写入；3:4 页面 PNG 只作为“成书预览”附在章节后，不能用一串整页截图替代正文。长文按原段落写入，照片使用原图，知识与会议只穿插少量解释图。写入后必须同时验收原生正文字符数、图片数量、块顺序和预览页数量；任一项缺失都不能标记发布完成。
