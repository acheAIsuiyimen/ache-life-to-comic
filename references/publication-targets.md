...
飞书月册默认采用连续画册模式，避免飞书原生排版与已验收页面形成两套互相冲突的阅读结构：

- 文档只保留月册标题与一句导语，随后按页码连续放置成品页，不增加重复章节标题、图注或正文块。
- 每页必须来自本地正式月册，固定为1080×1440；飞书显示宽度720，高度按3:4保持，不裁切、不二次套框。
- 上传前验证页数、页名顺序、尺寸、文件大小、SHA-256唯一性，以及源HTML桌面与手机QA；缺页、重复、乱序、低清或比例错误一律阻塞。
- 写入后回读图片数量与`name`顺序，并核对warnings/degrade和revision。飞书发生转码不影响通过，但页名和页序不得改变。
- 用户需要检索、复制或无障碍文字时，提供独立文字附录或原文入口；不要把重复文字插在成品页之间打断阅读。
